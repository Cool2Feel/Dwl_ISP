...
#define LSC_GRID_X 64
#define LSC_GRID_Y 32
...
struct iq_config{
	...
	unsigned int	lsc_ref_x;            // 参考点X坐标
	unsigned int	lsc_ref_y;            // 参考点Y坐标
	char			lsc_weight_file[256]; // 权重文件路径
	...
	// 动态计算的块大小 (每帧更新)
	unsigned int	lsc_block_size_x;
	unsigned int	lsc_block_size_y;
