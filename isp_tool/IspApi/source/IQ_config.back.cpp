...
void IqConfig(iq_config *iq_cfg, iq_image_buff *iq_img_buff, bool isp_en, bool isp_cal_en) {
	...
	// 根据分辨率设置LSC参数
	if (iq_cfg->image_height == 720) {
		iq_cfg->lsc_block_size_x = 64;
		iq_cfg->lsc_block_size_y = 32;
		strcpy(iq_cfg->lsc_weight_file, "./input/outweight32x64_720.txt");
	}
	else if (iq_cfg->image_height == 1080) {
		iq_cfg->lsc_block_size_x = 96;   // 按1920/1280比例调整
		iq_cfg->lsc_block_size_y = 48;
		strcpy(iq_cfg->lsc_weight_file, "./input/outweight48x96_1080.txt");
	}
	else if (iq_cfg->image_height == 1440) {
		iq_cfg->lsc_block_size_x = 128;
		iq_cfg->lsc_block_size_y = 64;
		strcpy(iq_cfg->lsc_weight_file, "./input/outweight64x128_1440.txt");
	}
	else {
		// 默认设置：按比例计算
		iq_cfg->lsc_block_size_x = (iq_cfg->image_width * 64) / 1280;
		iq_cfg->lsc_block_size_y = (iq_cfg->image_height * 32) / 720;
		// 生成对应的权重文件名
		sprintf(iq_cfg->lsc_weight_file, "./input/outweight%d_%d_%d.txt",
			iq_cfg->lsc_block_size_y, iq_cfg->lsc_block_size_x, iq_cfg->image_height);
	}

	// 设置参考点为图像中心
	iq_cfg->lsc_ref_x = iq_cfg->image_width / 2;
	iq_cfg->lsc_ref_y = iq_cfg->image_height / 2;

	// 动态加载权重文件
	FILE* fp = NULL;
	fp = fopen(iq_cfg->lsc_weight_file, "rt");
	if (fp == NULL) {
		printf("Cannot find LSC table for resolution %dx%d, generating new one...\n",
			iq_cfg->image_width, iq_cfg->image_height);
		// 如果文件不存在，切换到计算模式
		//iq_cfg->lsc_cal_en = 1;
	}
	else {
