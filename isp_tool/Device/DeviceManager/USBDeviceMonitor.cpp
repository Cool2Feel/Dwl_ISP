...
#include <initguid.h>
#include <winioctl.h>
#include <Wiaintfc.h> // for GUID_DEVINTERFACE_IMAGE
#include <ks.h> // for KSCATEGORY_CAPTURE
...
int USBDeviceMonitor::StartMonitor()
{
    ...
    const GUID* g_monitorDevices[] = {
        &KSCATEGORY_CAPTURE,       // 摄像头
        &GUID_DEVINTERFACE_DISK   // U盘
    };
    ...
    //NotificationFilter.dbcc_classguid = GUID_DEVINTERFACE_IMAGE; // 👈 关键修改！
    // 批量注册所有设备监听
    //for (int i = 0; i < _countof(g_monitorDevices); i++) {
    //    DEV_BROADCAST_DEVICEINTERFACE filter = { 0 };
    //    filter.dbcc_size = sizeof(DEV_BROADCAST_DEVICEINTERFACE);
    //    filter.dbcc_devicetype = DBT_DEVTYP_DEVICEINTERFACE;
    //    filter.dbcc_classguid = *g_monitorDevices[i];
    //    //RegisterDeviceNotification(m_MessageWindow, &filter, DEVICE_NOTIFY_WINDOW_HANDLE);
    //    m_hDevNotify = RegisterDeviceNotification(m_MessageWindow, &filter, DEVICE_NOTIFY_WINDOW_HANDLE);
    //    if (!m_hDevNotify)
    //    {
    //        return -2;
    //    }
