...
extern HINSTANCE m_hInst;			//Ĭ����Դ
extern HINSTANCE m_hInst_Lang;		//����
...
#define MYWM_PRINT_LOGR ::RegisterWindowMessage(_T("PRINT_LOG_RELEASE"))  //��ʾĳ��״̬�����Զ��ͷ��ڴ�
#define SET_PER(x) ::SendMessage(hMainWnd, MYWM_SET_PERCENT, (WPARAM)(x), (LPARAM)_index);

//printf_log ����ʾ��log����Ҫ��֤ȫ�̵��ٽ���
int pritf_index;
...
void AX327X::CheckCapacity(void)
{
	...
	if (flashsize > codelength)				//ʣ��ռ����һ��Ž��м�⡣��ֹд����Դ
	{
		...
		{
		//	//throw new TargetMsg(ERR_SPI_SIZE, "FlashLib��������");
			//throw new TargetMsg(ERR_SPI_SIZE, _T("FlashLib ERROR:ID-9F=%08X; ID-AB=%08X; ID-90=%08X; ID-15=%08X; LIBNUM=%d;"), 
		  flash_ids[0], flash_ids[1], flash_ids[2], flash_ids[3], lib_id);
		}
	}	
...
void AX327X::SetCRC(char *srcData,int srcDataLen)
{
	...
	sec_num = (BYTE*)(srcData + srcData[9] * 16 + 0x14); // ����������
	crc_start = ((sec_num[3]<<24)|(sec_num[2]<<16)|(sec_num[1]<<8)|sec_num[0])<<9;
	sec_num = (BYTE*)(srcData + srcData[9] * 16 + 0x18); // ������������
...
BOOL AX327X::SetEncryptAddr(char *srcData,int srcDataLen)
{
	...
	if (  srcData[0] == 0 && 
		  ...
		  srcData[7] == 'R' && 
		  srcData[0x1fe] == 0x55 && 
		  srcData[0x1ff] == (char)0xAA         //bootsect��־λ�ж�
		)  
	{
       //���Խ�checksum��crc�ŵ���һ��
		SetCRC(srcData,srcDataLen);

		if(srcData[10] & (1<<4)){                //���ܱ�־λ�ж�
			srcData[10] |= 0x80; // �޸ļ��ܱ�־
			sec_num = (BYTE*)(srcData + srcData[9] * 16 + 0x14); // ����������
...
int AX327X::DownBinCode()
{
	...
	if(!SetEncryptAddr(_down_code,_down_code_len)){
		//throw new Msg(ERR_CFG, IDS_LOG_TARGET_OPENERR);//��Ŀ���ļ�����
	}
	...
	if (_down_code_len) {
		...
		{
			SpiErase(0, SPI_BLOCK_SIZE);	//У��ʧ�������0 Block���ٵ�����
...
if(1){	//��SPI FlashУ��
	...
	for (i=512; i<_down_code_len-512; i+=SPI_BLOCK_SIZE) //ȫ������ 
	{
...
int AX327X::PreProbe()
{
	...
	if (  _rd_buf[0] == 0 && 
		  ...
		  _rd_buf[7] == 'R' && 
		  _rd_buf[0x1fe] == 0x55 && 
		  _rd_buf[0x1ff] == (char)0xAA         //bootsect��־λ�ж�
		  ) {
...
//		CXmlDataInfo LCDXml;
//
//		pLCD_Param = new UINT[LCDXml.GetValidModule(_T("LCD"))];//�����ܸ���
...
//	CXmlDataInfo ISPXml;
//	ISPXml.AllMod = ISPXml.GetValidModule(_T("ISP"));//Ĭ�ϴ򿪵�ģ��
...
//	{
//		//throw new Msg(ERR_CFG, IDS_LOG_TARGET_OPENERR);		//��Ŀ���ļ�����
//		throw new Msg(ERR_CFG, _T("���ļ�����"));		//��Ŀ���ļ�����
//		return;
//	}
//	
//	fseek(fpin, 0, SEEK_END);
...
//	{
//		//throw new Msg(ERR_CFG, IDS_LOG_TARGET_LARGE);			//Ŀ���ļ�����
// 	}
//	
//	if(_down_code != NULL)
//		delete _down_code;
...
//	{
//		//throw new TargetMsg(ERR_CFG, IDS_LOG_TARGET_LONG);		//Ŀ���ļ��ļ���������洢���̫��
//	}
//
...
//		//throw new TargetMsg(ERR_CFG, IDS_LOG_TARGET_MUST_BINCOD);	//Ŀ���ļ�������bin����cod
...
//		return;
//		////throw new TargetMsg(ERR_CFG, IDS_LOG_TARGET_OPENERR);		//��Ŀ���ļ�����
//	}
//	
//	fseek(fpin, 0, SEEK_END);
//	_down_code_len = ftell(fpin);
...
//	if (_down_code_len > 0x2000000)
//	{
//		////throw new TargetMsg(ERR_CFG, IDS_LOG_TARGET_LARGE);			//Ŀ���ļ�����
// 	}
//	
//	if(_down_code != NULL)
//		delete _down_code;
...
bool AX327X::SetSensorMode(int param, const char* data, int dataSize)
{
	USB_CMD  UsbCmd = { 0 };
